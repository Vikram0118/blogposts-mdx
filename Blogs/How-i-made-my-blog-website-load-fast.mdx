...
While it's common to consider numerous best practices for projects, they alone may not be the magic formula for boosting performance. Optimization isn't just another checkbox in our workflow; it's the outcome of a smart workflow and the best methodologies. When developing applications, focus on key elements like 'Prefetching,' 'Incremental Rendering,' 'Search Engine Optimization,' and 'Handling the Initial Page Load' to ensure an intuitively optimized performance.
...
[Largest Contentful Paint (LCP)](https://web.dev/articles/lcp) metric reports the render time of the largest image or text block visible within the viewport, relative to when the user first navigated to the page. To provide a good user experience, sites should strive to have LCP of 2.5 seconds or less. 


In addition to LCP we also have to consider about [First Contentful Paint (FCP)](https://web.dev/articles/fcp). The First Contentful Paint (FCP) metric measures the time from when the user first navigated to the page to when any part of the page's content is rendered on the screen.To provide a good user experience, sites should have a FCP of 1.8 seconds or less.


[The First Input Delay (FID)](https://web.dev/articles/fid) metric helps measure your user's first impression of your site's interactivity and responsiveness. FID measures the time from when a user first interacts with a page (that is, when they click a link, tap on a button, or use a custom, JavaScript-powered control) to the time when the browser is actually able to begin processing event handlers in response to that interaction. To provide a good user experience, sites should strive to have a FID of 100 milliseconds or less.
